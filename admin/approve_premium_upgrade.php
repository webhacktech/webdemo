...
// 1. Approve upgrade request
$update = $conn->prepare("UPDATE vendor_premium_upgrades SET status = 'approved', approved_at = NOW() WHERE id = ?");
$update->bind_param("i", $upgrade_id);
$update->execute();
$update->close();

// 2. Update vendor to premium
$conn->query("UPDATE vendors SET subscription = 'premium' WHERE id = $vendor_id");
...
$action = "Approved premium upgrade for vendor ID $vendor_id";
$log->bind_param("is", $admin_id, $action);
$log->execute();
$log->close();

// 4. Optional: send email/notification to vendor (future)

// Redirect back
header("Location: admin_premium_upgrades.php?msg=Upgrade approved");
exit;