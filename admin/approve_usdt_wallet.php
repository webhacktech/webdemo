...
$stmt = $conn->prepare("UPDATE vendor_wallets SET status = 'approved', reviewed_at = NOW(), reviewer_admin_id = ? WHERE id = ?");
$stmt->bind_param("ii", $admin_id, $wallet_id);
if ($stmt->execute()) {
    $_SESSION['wallet_msg'] = "✅ Wallet approved successfully.";
} else {
    $_SESSION['wallet_msg'] = "❌ Failed to approve wallet.";
}
$stmt->close();
